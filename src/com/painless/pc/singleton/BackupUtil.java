...
public class BackupUtil {
  ...
  /**
   * Imports a backup from a content URI (Android SAF / ACTION_OPEN_DOCUMENT).
   * Copies the URI stream to a temporary file and delegates to importBackup().
   */
  public static boolean importBackupFromUri(android.net.Uri uri, Context context, int widgetId) {
    java.io.File tmp = null;
    try {
      tmp = java.io.File.createTempFile("pt_restore", ".zip", context.getCacheDir());
      try (InputStream in = context.getContentResolver().openInputStream(uri);
           java.io.FileOutputStream out = new java.io.FileOutputStream(tmp)) {
        copy(in, out);
      }
      return importBackup(tmp.getAbsolutePath(), context, widgetId);
    } catch (Exception e) {
      Debug.log(e);
      return false;
    } finally {

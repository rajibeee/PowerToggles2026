...
public class PCWidgetActivity extends AppWidgetProvider {
  ...
  // ++++++++++++++ StatusBar Widget ++++++++++++++++++++++
  private static final String NOTIFY_CHANNEL_ID = "pt_notify_channel";

  private static void ensureNotifyChannel(Context context) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      NotificationManager nm = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
      if (nm.getNotificationChannel(NOTIFY_CHANNEL_ID) == null) {
        NotificationChannel channel = new NotificationChannel(
            NOTIFY_CHANNEL_ID,
            "PowerToggles Status Bar",
            NotificationManager.IMPORTANCE_LOW);
        channel.setDescription("Status bar toggle widget");
  ...
  static final boolean updateStatusbarWidget(Context context) {
    ...
    final RemoteViews collapsedViews = new RemoteViews(context.getPackageName(), R.layout.nw_collapsed);
    collapsedViews.removeAllViews(R.id.big_notify_wrapper_1);
    collapsedViews.addView(R.id.big_notify_wrapper_1, nwViews.clone());

    Notification.Builder builder = new Notification.Builder(context)
        .setSmallIcon(NotifyUtil.getDrawable(icon))
        .setOngoing(true)
        .setWhen(System.currentTimeMillis())
        .setPriority(priority - 3)
        .setContent(collapsedViews);

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      builder.setChannelId(NOTIFY_CHANNEL_ID);
    }
    if (Globals.IS_LOLLIPOP) {
      builder.setVisibility(NotifyStatus.notifyVisibility(context));
    }

    boolean rowTwoBattery = false;
    ...
    // Build second row (big content view) after notification is built
    rowTwoBattery = buildSecondRow(notification, context, nwViews);
    ...
    return setting.batteryEnabled | rowTwoBattery || (icon > 3 && icon < 9); // Battery icon range
  }

  private static boolean buildSecondRow(Notification notification, Context context, RemoteViews nwViews) {

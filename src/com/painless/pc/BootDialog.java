  ...
  @Thunk final String[] COMMANDS = new String[] {
    ...
    if (getIntent().getBooleanExtra("info", false)) {
      new AlertDialog.Builder(this)
          .setTitle(R.string.bt_error_title)
          .setMessage(R.string.bt_error_summary)
      ...
      new AlertDialog.Builder(this)
          .setTitle(R.string.bt_menu_title)
          .setAdapter(adapter, (dialog, which) -> {
              rebootMode = which > 0;
              runSuCommand(which);
          })
          .setNegativeButton(R.string.act_cancel, null)
          .setOnDismissListener(d -> { if (!blockFinish) finish(); })
          .show();

    } else {
      rebootMode = getIntent().getBooleanExtra("restart", false);
      new AlertDialog.Builder(this)
          .setTitle(rebootMode ? R.string.bt_restart : R.string.bt_poweroff)
          .setMessage(rebootMode ? R.string.bt_restart_summary : R.string.bt_poweroff_summary)
          .setPositiveButton(R.string.act_ok, (dialog, which) -> runSuCommand(rebootMode ? 1 : 0))
          .setNegativeButton(R.string.act_cancel, null)
          .setOnDismissListener(d -> { if (!blockFinish) finish(); })
          .show();
    }
